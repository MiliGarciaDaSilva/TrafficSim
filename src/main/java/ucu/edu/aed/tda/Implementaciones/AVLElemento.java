...
public class AVLElemento<T> implements TDAElemento<T> {
    ...
    /**
     * Elimina un nodo según un criterio y devuelve la raíz del subárbol balanceado.
     *
     * <p>Maneja tres casos:
     * <ul>
     *   <li><b>Nodo sin hijos (hoja)</b>: devuelve null.</li>
     *   <li><b>Nodo con un hijo</b>: devuelve ese hijo.</li>
     *   <li><b>Nodo con dos hijos</b>: busca el sucesor (mínimo del subárbol derecho),
     *       reemplaza el dato de este nodo con el del sucesor y elimina recursivamente el sucesor.</li>
     * </ul>
     * Tras cada eliminación se rebalancea el subárbol.</p>
     *
     * @param criterioBusqueda criterio que indica qué nodo eliminar
     * @return la nueva raíz del subárbol después de eliminar y balancear, o null si se eliminó este nodo
     */
    public AVLElemento<T> eliminarBalanceado(Comparable<T> criterioBusqueda) {
        int cmp = criterioBusqueda.compareTo(this.dato);

        if (cmp < 0) {
            if (hijoIzquierdo == null) return this;
            hijoIzquierdo = ((AVLElemento<T>) hijoIzquierdo).eliminarBalanceado(criterioBusqueda);
        } else if (cmp > 0) {
            if (hijoDerecho == null) return this;
            hijoDerecho = ((AVLElemento<T>) hijoDerecho).eliminarBalanceado(criterioBusqueda);
        } else {
            ...
            if (hijoIzquierdo == null) {
                return (AVLElemento<T>) hijoDerecho;
            }
            if (hijoDerecho == null) {
                return (AVLElemento<T>) hijoIzquierdo;
            }
            // Caso 3: nodo con dos hijos
            // Buscar el sucesor (mínimo del subárbol derecho)
            AVLElemento<T> sucesor = (AVLElemento<T>) hijoDerecho;
            while (sucesor.hijoIzquierdo != null) {
                sucesor = (AVLElemento<T>) sucesor.hijoIzquierdo;
            }
            // Reemplazar el dato de este nodo con el del sucesor
            this.dato = sucesor.dato;
            // Eliminar el sucesor del subárbol derecho
            hijoDerecho = ((AVLElemento<T>) hijoDerecho).eliminarBalanceado(new ComparableWrapper(sucesor.dato));
        }
    ...
    /**
     * Clase auxiliar para envolver un objeto T como Comparable<T> para eliminar el sucesor.
     */
    private class ComparableWrapper implements Comparable<T> {
        private T valor;

        public ComparableWrapper(T valor) {
            this.valor = valor;
        }
        ...
        public int compareTo(T otro) {
            return ((Comparable<T>) this.valor).compareTo(otro);
        }
    }
}
