...
public class AVLArbol<T> implements TDAArbolBinario<T> {
     ...
     *
     * <p>Si el nodo existe, lo elimina manteniendo la propiedad AVL mediante rebalanceo.
     * Maneja los tres casos estándar de eliminación en árboles binarios:
     * nodos sin hijos, nodos con un hijo, y nodos con dos hijos.</p>
     *
     * @param criterioBusqueda criterio que indica qué nodo eliminar
     * @return {@code true} si se eliminó el nodo; {@code false} si no existía
     */
    @Override
    public boolean eliminar(Comparable<T> criterioBusqueda) {
        if (raiz == null) return false;
        if (raiz.buscar(criterioBusqueda) == null) return false;
        raiz = raiz.eliminarBalanceado(criterioBusqueda);
        return true;
    }
